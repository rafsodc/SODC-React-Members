...
const Basket = (props) => {
  ...
  const tickets = basketState.tickets.map((ticket, key) => {
    return <p key={key}>
      <em>{ticket.ticketType.description} - &pound;{ticket.ticketType.price} - {ticket.lastname}, {ticket.firstname}</em>
    </p>
  })

  const content = (
    <Aux><br/>
      <p>Please review the items for purchase below. Items that have already been purchased will not be shown. If you
        believe you have already paid for an item, please email <a href="mailto:admin@sodc.net">admin@sodc.net</a>.</p>
